...
const Contact = ({ selectedService }) => {
  ...
  useEffect(() => {
    async function getData() {
      ...
      // console.log(response.data.fetchallcategory, "check getData");
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const whatsappMessage = `Hello, *The Renuka Trips Goa*, I would like to know about the following services:
...
*My details are -*
  ...
  *Message:* ${formData.message}`;
    ...
    const whatsappURL = `https://wa.me/7410150610?text=${encodeURIComponent(
      whatsappMessage
    )}`;
      ...
      <div className="bg-white shadow-lg rounded-lg w-[90%] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="col-span-1 p-5 lg:p-8 shadow-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              <option value="" disabled>
                Select a service
                  ...
                  {fetchallcategory && fetchallcategory.map((e, index) => (
                <option
                  key={index}
                  value={e.subcategory}
                  disabled={formData.services.includes(e.subcategory)}
                >
                  {e.subcategory}
                </option>
              ))}
            ...
            <div className="mb-4 grid grid-cols-1 md:grid-cols-2 gap-2 max-h-28 overflow-y-auto">
              {formData.services.map((service) => (
                <div
                  key={service}
                  className="flex items-center justify-between border border-blue-500 text-blue-500 px-4 py-2 rounded-xl font-medium"
                >
                  <span>{service}</span>
                  <button
                    type="button"
                    onClick={() => handleServiceRemove(service)}
                    className="ml-1 text-white font-bold p-1 bg-blue-500 rounded-full"
                  >
                    <X className="size-3" />
                  </button>
                </div>
